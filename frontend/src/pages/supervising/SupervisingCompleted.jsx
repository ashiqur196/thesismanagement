import React, { useState, useEffect, useRef } from "react";
import { supervisorManagementService } from "../../services/supervisorManagement";
import { getProfileImage } from "../../services/getPicture";
import {
  Card,
  CardContent,
  CardDescription,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Skeleton } from "@/components/ui/skeleton";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
  TooltipTrigger,
} from "@/components/ui/tooltip";
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from "@/components/ui/select";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
} from "@/components/ui/popover";
import {
  Search,
  Users,
  Calendar as CalendarIcon,
  BookOpen,
  ChevronLeft,
  ChevronRight,
  FileText,
  RefreshCw,
  AlertCircle,
  Filter,
  X,
  ChevronDown,
  ChevronUp,
} from "lucide-react";
import { useNavigate } from "react-router-dom";

function SupervisingCompleted() {
  const navigate = useNavigate();
  const [theses, setTheses] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [searchQuery, setSearchQuery] = useState("");
  const [sortOrder, setSortOrder] = useState("desc");
  const [selectedMonth, setSelectedMonth] = useState("");
  const [selectedYear, setSelectedYear] = useState("");
  const [showYearPicker, setShowYearPicker] = useState(false);
  const [showMonthPicker, setShowMonthPicker] = useState(false);
  const [visibleYears, setVisibleYears] = useState([]);
  const [yearPage, setYearPage] = useState(1);
  const yearListRef = useRef(null);
  const [pagination, setPagination] = useState({
    page: 1,
    totalPages: 1,
    totalItems: 0,
    limit: 10,
    hasNext: false,
    hasPrev: false,
  });

  // Months array
  const months = [
    { value: "01", label: "January" },
    { value: "02", label: "February" },
    { value: "03", label: "March" },
    { value: "04", label: "April" },
    { value: "05", label: "May" },
    { value: "06", label: "June" },
    { value: "07", label: "July" },
    { value: "08", label: "August" },
    { value: "09", label: "September" },
    { value: "10", label: "October" },
    { value: "11", label: "November" },
    { value: "12", label: "December" },
  ];

  // Initialize years with current decade
  useEffect(() => {
    const currentYear = new Date().getFullYear();
    const startYear = currentYear - 5;
    const initialYears = Array.from({ length: 20 }, (_, i) => startYear + i);
    setVisibleYears(initialYears);
  }, []);

  // Load more years when scrolling
  const loadMoreYears = () => {
    const lastYear = visibleYears[visibleYears.length - 1];
    const newYears = Array.from({ length: 20 }, (_, i) => lastYear + i + 1);
    setVisibleYears((prev) => [...prev, ...newYears]);
    setYearPage((prev) => prev + 1);
  };

  // Load previous years
  const loadPreviousYears = () => {
    const firstYear = visibleYears[0];
    const newYears = Array.from({ length: 20 }, (_, i) => firstYear - 20 + i);
    setVisibleYears((prev) => [...newYears, ...prev]);
  };

  // Debounce search to avoid too many API calls
  useEffect(() => {
    const timer = setTimeout(() => {
      if (searchQuery !== undefined) {
        fetchCompletedTheses(
          1,
          searchQuery,
          sortOrder,
          selectedYear,
          selectedMonth
        );
      }
    }, 500);

    return () => clearTimeout(timer);
  }, [searchQuery]);

  // Auto-apply filters when they change
  useEffect(() => {
    fetchCompletedTheses(
      1,
      searchQuery,
      sortOrder,
      selectedYear,
      selectedMonth
    );
  }, [sortOrder, selectedYear, selectedMonth]);

  const fetchCompletedTheses = async (
    page = 1,
    search = "",
    order = "desc",
    year = "",
    month = ""
  ) => {
    try {
      setLoading(true);
      setError(null);

      // Prepare query parameters
      const queryParams = {
        page,
        limit: 10,
        search,
        sort: order,
      };

      // Add date filters if selected
      if (year) queryParams.year = year;
      if (month) queryParams.month = month;

      const data = await supervisorManagementService.getCompletedTheses(
        queryParams
      );

      if (data.success) {
        setTheses(data.data.theses || []);
        setPagination(
          data.data.pagination || {
            page: 1,
            totalPages: 1,
            totalItems: 0,
            limit: 10,
            hasNext: false,
            hasPrev: false,
          }
        );
      } else {
        setError(data.message || "Failed to fetch completed theses");
      }
    } catch (err) {
      setError(
        err.message || "An error occurred while fetching completed theses"
      );
    } finally {
      setLoading(false);
    }
  };

  const handlePageChange = (newPage) => {
    if (newPage >= 1 && newPage <= pagination.totalPages) {
      fetchCompletedTheses(
        newPage,
        searchQuery,
        sortOrder,
        selectedYear,
        selectedMonth
      );
    }
  };

  const clearFilters = () => {
    setSearchQuery("");
    setSortOrder("desc");
    setSelectedYear("");
    setSelectedMonth("");
    fetchCompletedTheses(1, "", "desc", "", "");
  };

  const formatDate = (dateString) => {
    return new Date(dateString).toLocaleDateString("en-US", {
      year: "numeric",
      month: "short",
      day: "numeric",
    });
  };

  const hasActiveFilters =
    searchQuery || selectedYear || selectedMonth || sortOrder !== "desc";

  const getDisplayText = () => {
    if (selectedYear && selectedMonth) {
      const month = months.find((m) => m.value === selectedMonth)?.label;
      return `${month} ${selectedYear}`;
    }
    if (selectedYear) return `Year: ${selectedYear}`;
    if (selectedMonth)
      return `Month: ${months.find((m) => m.value === selectedMonth)?.label}`;
    return "Select month/year";
  };

  if (loading && theses.length === 0) {
    return (
      <div className="container mx-auto">
        <div className="mb-6">
          <h1 className="text-3xl font-bold tracking-tight">
            Completed Theses
          </h1>
          <p className="text-muted-foreground mt-2">
            Theses you have previously supervised
          </p>
        </div>

        <div className="mb-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Skeleton className="h-10 w-full" />
          <Skeleton className="h-10 w-full" />
          <Skeleton className="h-10 w-full" />
          <Skeleton className="h-10 w-full" />
        </div>

        <div className="grid gap-4">
          {[1, 2, 3].map((i) => (
            <Card key={i} className="overflow-hidden">
              <CardHeader className="pb-3">
                <Skeleton className="h-6 w-3/4 mb-2" />
                <div className="flex gap-2">
                  <Skeleton className="h-5 w-16" />
                  <Skeleton className="h-5 w-16" />
                </div>
              </CardHeader>
              <CardContent>
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <Skeleton className="h-6 w-6 rounded-full" />
                    <Skeleton className="h-6 w-6 rounded-full" />
                    <Skeleton className="h-6 w-6 rounded-full" />
                  </div>
                  <Skeleton className="h-4 w-20" />
                </div>
              </CardContent>
            </Card>
          ))}
        </div>
      </div>
    );
  }

  if (error) {
    return (
      <div className="container mx-auto">
        <Card className="border-destructive">
          <CardHeader className="text-destructive">
            <CardTitle className="flex items-center gap-2">
              <AlertCircle className="h-5 w-5" />
              Error
            </CardTitle>
          </CardHeader>
          <CardContent>
            <p className="mb-4">{error}</p>
            <Button
              onClick={() =>
                fetchCompletedTheses(1, searchQuery, sortOrder, date)
              }
              className="flex items-center gap-2"
            >
              <RefreshCw className="h-4 w-4" />
              Try Again
            </Button>
          </CardContent>
        </Card>
      </div>
    );
  }

  return (
    <div className="container mx-auto">
      {/* Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-primary to-primary/60 bg-clip-text text-transparent">
            Completed Theses
          </h1>
          <p className="text-muted-foreground mt-2">
            {theses.length > 0
              ? `You have supervised ${pagination.totalItems} completed thesis${
                  pagination.totalItems !== 1 ? "es" : ""
                }`
              : "No completed theses found"}
          </p>
        </div>
        <Button
          variant="outline"
          onClick={() =>
            fetchCompletedTheses(
              1,
              searchQuery,
              sortOrder,
              selectedYear,
              selectedMonth
            )
          }
          className="flex items-center gap-2"
        >
          <RefreshCw className="h-4 w-4" />
          Refresh
        </Button>
      </div>

      {/* Filters Section */}
      <Card className="mb-6">
        <CardHeader className="pb-3">
          <CardTitle className="text-lg flex items-center gap-2">
            <Filter className="h-5 w-5" />
            Filters
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {/* Search Input */}
            <div className="relative">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground h-4 w-4" />
              <Input
                type="text"
                placeholder="Search theses..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="pl-10"
              />
            </div>

            {/* Sort Order */}
            <Select value={sortOrder} onValueChange={setSortOrder}>
              <SelectTrigger>
                <SelectValue placeholder="Sort by" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="desc">Newest First</SelectItem>
                <SelectItem value="asc">Oldest First</SelectItem>
              </SelectContent>
            </Select>

            {/* Month/Year Picker */}
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" className="w-full justify-between">
                  <span className="truncate">{getDisplayText()}</span>
                  <CalendarIcon className="ml-2 h-4 w-4" />
                </Button>
              </PopoverTrigger>
              <PopoverContent className="p-0 w-64" align="start">
                {/* Show Year Picker OR Month Picker */}
                {!showMonthPicker ? (
                  <div className="flex flex-col">
                    {/* Year Picker Header */}
                    <div className="p-2 border-b">
                      <div className="flex justify-between items-center">
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={loadPreviousYears}
                          className="h-7 w-7 p-0"
                        >
                          <ChevronUp className="h-4 w-4" />
                        </Button>
                        <span className="text-sm font-medium">Year</span>
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={loadMoreYears}
                          className="h-7 w-7 p-0"
                        >
                          <ChevronDown className="h-4 w-4" />
                        </Button>
                      </div>
                    </div>

                    {/* Year List */}
                    <div
                      className="max-h-60 overflow-y-auto p-2"
                      ref={yearListRef}
                    >
                      {visibleYears.map((year) => (
                        <Button
                          key={year}
                          variant={
                            selectedYear === year.toString()
                              ? "default"
                              : "ghost"
                          }
                          className="w-full justify-start mb-1"
                          onClick={() => {
                            setSelectedYear(year.toString());
                            setShowMonthPicker(true);
                          }}
                        >
                          {year}
                        </Button>
                      ))}
                    </div>
                  </div>
                ) : (
                  <div className="flex flex-col">
                    {/* Month Picker Header */}
                    <div className="p-2 border-b">
                      <div className="flex justify-between items-center">
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => setShowMonthPicker(false)}
                          className="h-7 w-7 p-0"
                        >
                          <ChevronLeft className="h-4 w-4" />
                        </Button>
                        <span className="text-sm font-medium">Month</span>
                        <div className="w-7" />
                      </div>
                    </div>

                    {/* Month Grid */}
                    <div className="max-h-64 p-2 overflow-y-auto grid grid-cols-2 gap-1">
                      {months.map((month) => (
                        <Button
                          key={month.value}
                          variant={
                            selectedMonth === month.value ? "default" : "ghost"
                          }
                          className="w-full justify-start"
                          onClick={() => {
                            setSelectedMonth(month.value);
                            
                          }}
                        >
                          {month.label}
                        </Button>
                      ))}
                    </div>
                  </div>
                )}
              </PopoverContent>
            </Popover>

            {/* Clear Filters Button */}
            {hasActiveFilters && (
              <Button
                variant="outline"
                onClick={clearFilters}
                className="flex items-center gap-2"
              >
                <X className="h-4 w-4" />
                Clear Filters
              </Button>
            )}
          </div>

          {/* Active Filters Badges */}
          {hasActiveFilters && (
            <div className="flex flex-wrap gap-2 mt-4">
              {searchQuery && (
                <Badge variant="secondary" className="flex items-center gap-1">
                  Search: {searchQuery}
                  <X
                    className="h-3 w-3 cursor-pointer"
                    onClick={() => setSearchQuery("")}
                  />
                </Badge>
              )}
              {selectedYear && (
                <Badge variant="secondary" className="flex items-center gap-1">
                  Year: {selectedYear}
                  <X
                    className="h-3 w-3 cursor-pointer"
                    onClick={() => setSelectedYear("")}
                  />
                </Badge>
              )}
              {selectedMonth && (
                <Badge variant="secondary" className="flex items-center gap-1">
                  Month: {months.find((m) => m.value === selectedMonth)?.label}
                  <X
                    className="h-3 w-3 cursor-pointer"
                    onClick={() => setSelectedMonth("")}
                  />
                </Badge>
              )}
              {sortOrder !== "desc" && (
                <Badge variant="secondary" className="flex items-center gap-1">
                  Sort: {sortOrder === "asc" ? "Oldest First" : "Newest First"}
                  <X
                    className="h-3 w-3 cursor-pointer"
                    onClick={() => setSortOrder("desc")}
                  />
                </Badge>
              )}
            </div>
          )}
        </CardContent>
      </Card>

      {/* Results */}
      {theses.length === 0 ? (
        <Card className="border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            <BookOpen className="h-20 w-20 text-muted-foreground/40 mb-4" />
            <h3 className="text-lg font-medium mb-2">
              {hasActiveFilters
                ? "No matching theses found"
                : "No completed theses"}
            </h3>
            <p className="text-muted-foreground max-w-md">
              {hasActiveFilters
                ? "Try adjusting your filters to see more results."
                : "Completed theses you have supervised will appear here."}
            </p>
            {hasActiveFilters && (
              <Button variant="outline" className="mt-4" onClick={clearFilters}>
                Clear All Filters
              </Button>
            )}
          </CardContent>
        </Card>
      ) : (
        <>
          <div className="grid gap-6">
            {theses.map((thesis) => (
              <Card
                key={thesis.id}
                className="overflow-hidden transition-all hover:shadow-lg border-l-4 border-l-green-500"
              >
                <CardHeader className="pb-3">
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <CardTitle className="text-xl mb-2 line-clamp-2">
                        {thesis.title}
                      </CardTitle>
                      <div className="flex items-center gap-3 flex-wrap">
                        <Badge
                          variant="outline"
                          className="capitalize bg-green-50 text-green-700 border-green-200"
                        >
                          {thesis.status.toLowerCase()}
                        </Badge>
                        <div className="flex items-center text-sm text-muted-foreground">
                          <CalendarIcon className="h-4 w-4 mr-1" />
                          <span>Completed {formatDate(thesis.updatedAt)}</span>
                        </div>
                        <div className="flex items-center text-sm text-muted-foreground">
                          <CalendarIcon className="h-4 w-4 mr-1" />
                          <span>Created {formatDate(thesis.createdAt)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                </CardHeader>

                <CardContent>
                  <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-6">
                    <div className="space-y-4 flex-1">
                      {thesis.researchTags &&
                        thesis.researchTags.length > 0 && (
                          <div className="flex flex-wrap gap-2">
                            {thesis.researchTags
                              .slice(0, 4)
                              .map((tag, index) => (
                                <Badge
                                  key={index}
                                  variant="secondary"
                                  className="text-xs py-1"
                                >
                                  {tag}
                                </Badge>
                              ))}
                            {thesis.researchTags.length > 4 && (
                              <Badge variant="outline" className="text-xs py-1">
                                +{thesis.researchTags.length - 4} more
                              </Badge>
                            )}
                          </div>
                        )}

                      <div className="flex items-center gap-4">
                        <Users className="h-4 w-4 text-muted-foreground flex-shrink-0" />
                        <div className="flex items-center">
                          <TooltipProvider>
                            <div className="flex -space-x-2">
                              {thesis.members.slice(0, 5).map((member) => (
                                <Tooltip key={member.id}>
                                  <TooltipTrigger asChild>
                                    <Avatar className="h-8 w-8 border-2 border-background hover:scale-110 transition-transform">
                                      <AvatarImage
                                        src={getProfileImage(member.user.image)}
                                        alt={member.name}
                                      />
                                      <AvatarFallback className="text-xs bg-green-100">
                                        {member.name
                                          .split(" ")
                                          .map((n) => n[0])
                                          .join("")}
                                      </AvatarFallback>
                                    </Avatar>
                                  </TooltipTrigger>
                                  <TooltipContent>
                                    <p>{member.name}</p>
                                  </TooltipContent>
                                </Tooltip>
                              ))}
                              {thesis.members.length > 5 && (
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <div className="h-8 w-8 rounded-full bg-muted flex items-center justify-center text-xs font-medium border-2 border-background">
                                      +{thesis.members.length - 5}
                                    </div>
                                  </TooltipTrigger>
                                  <TooltipContent>
                                    <p>
                                      And {thesis.members.length - 5} more
                                      members
                                    </p>
                                  </TooltipContent>
                                </Tooltip>
                              )}
                            </div>
                          </TooltipProvider>
                          <span className="text-sm text-muted-foreground ml-2">
                            {thesis.members.length} member
                            {thesis.members.length !== 1 ? "s" : ""}
                          </span>
                        </div>
                      </div>
                    </div>

                    <div className="flex gap-2 self-start lg:self-center">
                      <Button
                        onClick={() =>
                          navigate(`/thesis/${thesis.id}`)
                        }
                        className="flex items-center gap-2"
                      >
                        <FileText className="h-4 w-4" />
                        View Details
                      </Button>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ))}
          </div>

          {/* Pagination */}
          {pagination.totalPages > 1 && (
            <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mt-8">
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
                  onClick={() => handlePageChange(pagination.page - 1)}
                  disabled={!pagination.hasPrev}
                  className="flex items-center gap-2"
                >
                  <ChevronLeft className="h-4 w-4" />
                  Previous
                </Button>

                <div className="flex items-center gap-1">
                  {Array.from(
                    { length: Math.min(5, pagination.totalPages) },
                    (_, i) => {
                      let pageNum;
                      if (pagination.totalPages <= 5) {
                        pageNum = i + 1;
                      } else if (pagination.page <= 3) {
                        pageNum = i + 1;
                      } else if (pagination.page >= pagination.totalPages - 2) {
                        pageNum = pagination.totalPages - 4 + i;
                      } else {
                        pageNum = pagination.page - 2 + i;
                      }

                      return (
                        <Button
                          key={pageNum}
                          variant={
                            pagination.page === pageNum ? "default" : "outline"
                          }
                          size="icon"
                          onClick={() => handlePageChange(pageNum)}
                          className="w-10 h-10"
                        >
                          {pageNum}
                        </Button>
                      );
                    }
                  )}

                  {pagination.totalPages > 5 &&
                    pagination.page < pagination.totalPages - 2 && (
                      <span className="px-2 text-muted-foreground">...</span>
                    )}
                </div>

                <Button
                  variant="outline"
                  onClick={() => handlePageChange(pagination.page + 1)}
                  disabled={!pagination.hasNext}
                  className="flex items-center gap-2"
                >
                  Next
                  <ChevronRight className="h-4 w-4" />
                </Button>
              </div>

              <span className="text-sm text-muted-foreground">
                Page {pagination.page} of {pagination.totalPages} •{" "}
                {pagination.totalItems} items
              </span>
            </div>
          )}
        </>
      )}
    </div>
  );
}

export default SupervisingCompleted;
